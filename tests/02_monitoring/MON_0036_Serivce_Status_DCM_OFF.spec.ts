...
test('MON_0036 Service status DCM OFF', async ({ page }) => {
    ...
    // DCM OFF 진행
    await control.controlService('DCM', 'off');
    // Monitoring 페이지 진입
    ...
    await expect(dcmButtonArea).toHaveScreenshot('dcm_off_red_area.png', { timeout: 8000 });
});
