...
test('MON_0053 Map View - Mini Map Close', async ({ page }, testInfo) => {
    ...
    if (!isMiniMapOpen) {        
        console.log('⚠️ 스크린샷 불일치: 미니맵이 열려있는 상태로 판정되어 우측 하단을 클릭합니다.');
        ...
        if (box) {
            ...
            await expect(mapCanvas).toHaveScreenshot('map_mini_map_close.png', {
        ...
        console.log('✅ 스크린샷 일치: 미니맵이 이미 정상적으로 닫혀 있습니다.');
    }
